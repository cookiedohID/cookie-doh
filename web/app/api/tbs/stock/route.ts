...
import { canSeeTbsShop, partnerGetStock } from "@/lib/tbsShop";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    // chunked + base-sku-expanded (cross-pack stock checks need base entries)
    const data = await partnerGetStock(store, skus.split(","));
