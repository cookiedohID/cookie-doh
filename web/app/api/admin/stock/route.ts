...
export async function GET() {
  try {
    ...
    const storesRes = await supa.from("store_locations").select("id,name").order("name");
...
export async function POST(req: Request) {
  try {
    ...
    if (!store_id || !flavor_id || !Number.isFinite(qty) || qty < 0) {
      return NextResponse.json({ ok: false, error: "Invalid store_id / flavor_id / qty" }, { status: 400 });
    }

    const { error } = await supa
      .from("flavor_stock")
      .upsert(
        { store_id, flavor_id, qty: Math.floor(qty), updated_at: new Date().toISOString() },
