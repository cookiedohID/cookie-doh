...
export async function POST(
  ...
  try {
    ...
    const { data: order, error: oErr } = await sb.from("orders").select("*").eq("id", id).single();
    ...
    if (lat == null || lng == null) {
      return NextResponse.json({ ok: false, error: "Missing destination lat/lng in shipping_json" }, { status: 400 });
    }

    const meta = parseMeta(order.meta);
    const origin = meta?.quote?.origin || null;

    if (!origin?.lat || !origin?.lng || !origin?.address || !origin?.name) {
      return NextResponse.json(
        { ok: false, error: "Missing stored origin for booking. Requote required on checkout." },
        { status: 400 }
      );
    }

    const serviceType = meta?.quote?.serviceType || process.env.LALAMOVE_SERVICE_INSTANT_PRIORITY;

    const llm = await createLalamoveOrder({
      externalId: String(order.midtrans_order_id ?? order.order_no ?? id),
      serviceType: String(serviceType || "MOTORCYCLE"),
      language: "id_ID",

      pickupAddress: String(origin.address),
      pickupLat: Number(origin.lat),
      pickupLng: Number(origin.lng),
      pickupContactName: String(origin.name),
      // You can add per-origin phones later. For now:
      pickupContactPhone: toE164ID(process.env.COOKIE_DOH_PICKUP_PHONE || "+628000000000"),
    ...
    return NextResponse.json(
      { ok: false, error: e?.message || "Lalamove request failed", status: e?.status || null, lalamove: e?.payload || null },
