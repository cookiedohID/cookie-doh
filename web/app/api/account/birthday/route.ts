...
export async function POST(req: Request) {
  try {
    ...
    let birthday = String(b?.birthday || "").trim();
    if (birthday) {
    ...
    const { error } = await supa.from("customers").update({ birthday: birthday || null }).eq("auth_user_id", user.id);
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 400 });
    return NextResponse.json({ ok: true, birthday: birthday || null });
  } catch (e: any) {
