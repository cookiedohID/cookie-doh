...
function isAuthorized(req: NextRequest) {
  const expected = getEnv("ADMIN_RETRY_SECRET");
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const midtrans_order_id = String(
      body?.midtrans_order_id ?? body?.order_id ?? ""
    ).trim();

    if (!midtrans_order_id) {
      return NextResponse.json(
        { ok: false, error: "Missing midtrans_order_id" },
        { status: 400 }
      );
    }
    ...
    const destination_address = String(
      order.shipping_address ?? order.address ?? ""
    ).trim();

    const destination_postal_code = order.postal
      ? Number(String(order.postal).trim())
      : null;

    const destination_area_id = order.destination_area_id
    ...
    if (!courier_company || !courier_type) {
      ...
      return NextResponse.json(
        { ok: false, error: "Missing courier info" },
        { status: 400 }
      );
    }
    ...
    if (!destination_contact_name || !destination_contact_phone || !destination_address) {
      ...
      return NextResponse.json(
        { ok: false, error: "Missing destination fields" },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      const biteship_order_id =
        biteshipRes?.id ?? biteshipRes?.data?.id ?? null;
