...
const STORE_ID = "kemang"; // ✅ fixed for go-live (store selector hidden)
...
export default function BuildClient({ initialBoxSize = 6 }: { initialBoxSize?: BoxSize }) {
  ...
  const cardFlavors: CardFlavorUI[] = useMemo(() => {
    return CATALOG_FLAVORS.map((f: any) => {
      ...
      // ✅ do NOT assume sold out while loading
  ...
  // clamp selections after stock arrives
  useEffect(() => {
        ...
        <header style={{ marginBottom: 14 }}>
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "baseline" }}>
              <div style={{ fontWeight: 950, color: COLORS.black }}>Stock base: Kemang</div>
              <a
