  ...
  return (
    <main style={{ minHeight: "100vh", background: COLORS.sand }}>
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "28px 18px 90px" }}>
        ...
        <Section title="💬 WhatsApp assistant (auto-reply)">
          <p style={p}>When a customer messages your WhatsApp, an <b>AI assistant answers instantly</b> (Claude Haiku) and <b>hands off to you</b> for anything it can&apos;t.</p>
          <ul style={{ paddingLeft: 18 }}>
            <li style={li}>Answers from your live menu + policies: prices, delivery/pickup, how to order, loyalty, subscriptions, referrals, birthdays, promo codes, gifts. It can look up <b>that customer&apos;s own</b> order status, tracking and reward balance (never anyone else&apos;s).</li>
            <li style={li}><b>Hand-off:</b> for complaints, refunds, changing/cancelling a paid order, custom/wholesale/event requests, allergy-critical questions, anything it can&apos;t answer, or when the customer asks for a person — it <b>WhatsApps you</b> the customer&apos;s number + message + reason, and <b>mutes itself for that chat</b> so you can take over.</li>
            <li style={li}><b>Mute the bot per customer</b> (<span style={code}>/admin/whatsapp</span> → 💬 WhatsApp chats): tap <b>Mute bot</b> on a conversation you want to handle yourself — others still get instant replies. Muted chats (by you or by a hand-off) <b>auto-resume ~2h after the chat goes quiet</b>. (Replies you type on your phone aren&apos;t visible here — WhatsApp doesn&apos;t forward your outgoing messages — which is why muting is a tap rather than automatic.)</li>
            <li style={li}>It never takes payment or confirms an order in chat — it points customers to the website.</li>
            <li style={li}><b>Setup (one-time):</b> set <span style={code}>ANTHROPIC_API_KEY</span> + <span style={code}>WA_INBOUND_SECRET</span> in Vercel, then point Fonnte&apos;s incoming webhook at <span style={code}>/api/whatsapp/inbound?key=…</span>. Without the API key, inbound messages are just forwarded to you. Opening hours live in <span style={code}>web/lib/waKnowledge.ts</span>.</li>
          </ul>
        </Section>
