// web/lib/catalog.ts


export type FlavorBadge =
  | "Bestseller"
  | "Signature"
  | "New"
  | "Limited"
  | "Classic"
  | "Fan Favorite"
  | "House Favorite"
  | "Bold Choice"
  | "Calm Pick"
  | "Romantic"
  | "Crowd Pleaser"
  | "Indulgent"
  | "Comfort Pick"
  | "Late Night Comfort"
  | "Refined"
  | "Bright Finish";

export type FlavorIntensity = {
  chocolate?: number; // 0-5
  sweetness?: number; // 0-5
};

export type Flavor = {
  id: string;
  name: string;
  description?: string;
  image?: string; // e.g. "/flavors/choco-chip.jpg"
  image2?: string; // optional second photo, shown on hover (falls back to a zoom of `image`)
  badges?: FlavorBadge[]; // e.g. ["Bestseller", "Signature"]
  tags?: string[]; // e.g. ["Chewy", "Chunky", "Nutty"]
  ingredients?: string[]; // ✅ array to match your data
  intensity?: FlavorIntensity;

  // optional (legacy / compatibility)
  soldOut?: boolean;
};

export const FLAVORS: Flavor[] = [
  // 1) The One
  {
    id: "the-one",
    name: "The One",
    description:
      "Decadent dark chocolate cookie studded with creamy white chocolate chips -- a bold, blissful contrast in every bite.",
    ingredients: [
      "European Butter",
      "White Belgian Chocolate",
      "Bourbon Vanilla",
      "Sea Salt Flakes",
    ],
    image: "/flavors/CxCookiedoh/the one.jpg",
    image2: "/flavors/CxCookiedoh/the one_B.jpg",
    badges: ["Bestseller", "Signature"],
    tags: ["Classic", "Buttery", "Comfort"],
  },

  // 2) The Other One
  {
    id: "the-other-one",
    name: "The Other One",
    description:
      "Sinfully rich chocolate cookie loaded with dark + milk chocolate chips -- pure bliss in every bite.",
    ingredients: [
      "European Butter",
      "Dark + Milk Belgian Chocolate",
      "Bourbon Vanilla",
      "Sea Salt Flakes",
    ],
    image: "/flavors/CxCookiedoh/the other one.jpg",
    image2: "/flavors/CxCookiedoh/the other one_B.jpg",
    badges: ["Fan Favorite"],
    tags: ["Earthy", "Creamy", "Chocolatey"],
  },

  // 3) Orange In The Dark
  {
    id: "orange-in-the-dark",
    name: "Orange In The Dark",
    description:
      "Rich, fudgy chocolate cookie packed with dark chocolate chips and a citrusy twist of orange peel -- decadence with a zing!",
    ingredients: [
      "European Butter",
      "French Orange Peel",
      "Dark Belgian Chocolate",
      "Bourbon Vanilla",
      "Sea Salt Flakes",
    ],
    image: "/flavors/CxCookiedoh/orange in the dark.jpg",
    image2: "/flavors/CxCookiedoh/orange in the dark_B.jpg",
    badges: ["Bold Choice"],
    tags: ["Citrus", "Dark", "Aromatic"],
  },

  // 4) Matcha Magic
  {
    id: "matcha-magic",
    name: "Matcha Magic",
    description:
      "Like a serene Japanese garden in cookie form : vibrant matcha, sweet chocolate clouds, and pure melt-in-your-mouth magic.",
    ingredients: [
      "European butter",
      "Japanese Matcha (Ceremonial Grade)",
      "White Belgian Chocolate",
      "Bourbon Vanilla",
    ],
    image: "/flavors/CxCookiedoh/matcha magic.jpg",
    image2: "/flavors/CxCookiedoh/matcha magic_B.jpg",
    badges: ["House Favorite"],
    tags: ["Earthy", "Aromatic", "Creamy"],
  },

  // 5) Lavender Hush
  {
    id: "lavender-hush",
    name: "Lavender Hush",
    description:
      "Like a cookie version of fairy tale : floral lavender whispers, creamy chocolate, and a crumb so tender it’s practically pixie made.",
    ingredients: [
      "European Butter",
      "Dried Lavender Flower",
      "White Belgian Chocolate",
      "Bourbon Vanilla",
    ],
    image: "/flavors/CxCookiedoh/lavender hush.jpg",
    image2: "/flavors/CxCookiedoh/lavender hush_B.jpg",
    badges: ["Calm Pick"],
    tags: ["Floral", "Soft", "Elegant"],
  },

  // 6) Rose Lullaby
  {
    id: "rose-lullaby",
    name: "Rose Lullaby",
    description:
      "Fragrant rose petals and warm vanilla, cradling a custard center that melts into memory.",
    ingredients: [
      "European Butter",
      "Dried Rose Petals",
      "Creamy Vanilla Crème Pâtissière",
      "White Belgian Chocolate",
      "Bourbon Vanilla",
    ],
    image: "/flavors/CxCookiedoh/rose lullaby.jpg",
    image2: "/flavors/CxCookiedoh/rose lullaby_B.jpg",
    badges: ["Romantic"],
    tags: ["Floral", "Creamy", "Nostalgic"],
  },

  // 7) Strawberry Kiss
  {
    id: "strawberry-kiss",
    name: "Strawberry Kiss",
    description:
      "Sweet strawberries drift through creamy white chocolate, soft as a blush and lingering like a first kiss.",
    ingredients: [
      "European Butter",
      "Freeze-Dried Strawberry",
      "White Belgian Chocolate",
      "Bourbon Vanilla",
    ],
    image: "/flavors/CxCookiedoh/strawberry kiss.jpg",
    image2: "/flavors/CxCookiedoh/strawberry kiss_B.jpg",
    badges: ["Crowd Pleaser"],
    tags: ["Fruity", "Sweet", "Romantic"],
  },

  // 8) Crimson Crush — HIDDEN until it has its own photo.
  //    Re-enable by uncommenting and adding image: "/flavors/crimson-crush.jpg".
  // {
  //   id: "crimson-crush",
  //   name: "Crimson Crush",
  //   description:
  //     "Sweet strawberries sink into deep milk chocolate, rich and lingering, like a kiss remembered long after.",
  //   ingredients: [
  //     "European Butter",
  //     "Freeze-Dried Strawberry",
  //     "Milk + Dark Belgian Chocolate",
  //     "Bourbon Vanilla",
  //   ],
  //   image: "/flavors/crimson-crush.jpg",
  //   badges: ["Indulgent"],
  //   tags: ["Fruity", "Dark", "Rich"],
  // },

  // 9) Velvety Red
  {
    id: "velvety-red",
    name: "Velvety Red",
    description:
      "Velvety red cocoa cookie swirled with tangy cream cheese - bold, creamy and utterly irresistible.",
    ingredients: [
      "European Butter",
      "Cream Cheese",
      "White Belgian Chocolate",
      "Bourbon Vanilla",
      "Sea Salt Flakes",
    ],
    image: "/flavors/CxCookiedoh/red velvet.jpg",
    image2: "/flavors/CxCookiedoh/red velvet_B.jpg",
    badges: ["Comfort Pick"],
    tags: ["Creamy", "Rich", "Indulgent"],
  },

  // 10) Midnight Crave
  {
    id: "midnight-crave",
    name: "Midnight Crave",
    description:
      "Soft, buttery oat cookie with caramelized raisins, kissed with cinnamon and brown sugar warmth - indulgence after dark.",
    ingredients: [
      "European Butter",
      "Oatmeal",
      "Californian Raisin",
      "Cinnamon",
      "Bourbon Vanilla",
      "Sea Salt Flakes",
    ],
    image: "/flavors/CxCookiedoh/midnight crave.jpg",
    image2: "/flavors/CxCookiedoh/midnight crave_B.jpg",
    badges: ["Late Night Comfort"],
    tags: ["Comfort", "Spiced", "Nostalgic"],
  },

  // 11) One Shade of Grey
  {
    id: "one-shade-of-grey",
    name: "One Shade of Grey",
    description:
      "Like a cozy London cafe in every bite : bold bergamot, sweet milk chocolate and butter softness. ~ Earl Grey.",
    ingredients: [
      "European Butter",
      "Earl Grey Tea",
      "Milk Belgian Chocolate",
      "Bourbon Vanilla",
    ],
    image: "/flavors/CxCookiedoh/earl grey.jpg",
    image2: "/flavors/CxCookiedoh/earl grey_B.jpg",
    badges: ["Refined"],
    tags: ["Tea", "Elegant", "Balanced"],
  },

  // 12) Ruby Glow
  {
    id: "ruby-glow",
    name: "Ruby Glow",
    description:
      "Buttery cookie with creamy white chocolate and bursts of cranberry - a little glow in every bite.",
    ingredients: [
      "European Butter",
      "USA Cranberry",
      "White Belgian Chocolate",
      "Bourbon Vanilla",
      "Sea Salt Flakes",
    ],
    image: "/flavors/CxCookiedoh/ruby glow.jpg",
    image2: "/flavors/CxCookiedoh/ruby glow_B.jpg",
    badges: ["Bright Finish"],
    tags: ["Fruity", "Fresh", "Uplifting"],
  },
];

export const BOX_PRICES: Record<3 | 6, number> = {
  3: 90000,
  6: 180000,
};

export const FREE_SHIPPING_THRESHOLD = 200000;

export function formatIDR(amount: number) {
  return new Intl.NumberFormat("id-ID").format(amount);
}
